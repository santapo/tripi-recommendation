...
class DataPreprocessingNew {
  ...
  connector.withSessionDo(session => {
    session.execute("DROP KEYSPACE IF EXISTS testkeyspace2")
    session.execute("CREATE KEYSPACE testkeyspace2 WITH replication = {'class':'SimpleStrategy', 'replication_factor':1}")
    session.execute("USE testkeyspace2")
    session.execute("CREATE TABLE testkeyspace2.mapping_root " +
      "(id text PRIMARY KEY," +
      " name text," +
      " address text," +
      " province_name text," +
    ...
    session.execute("CREATE TABLE testkeyspace2.hotel_table " +
      "(id text PRIMARY KEY," +
      " hotel_cluster int," +
      " name text," +
      " address text," +
...
//      .load()
    ...
    hotel_mapping_with_price_clean.createCassandraTable("testkeyspace2", "hotel_mapping")
    ...
    hotel_service_table.createCassandraTable("testkeyspace2", "hotel_service")
    hotel_service_table
      .write
      .format("org.apache.spark.sql.cassandra")
      .mode("Append")
...
//      )
    ...
    roothotel_final_clean.createCassandraTable("testkeyspace2", "root_hotel")
    ...
    hotel_review_clean.createCassandraTable("testkeyspace2","hotel_review")
    hotel_review_clean
      .write
      .format("org.apache.spark.sql.cassandra")
      .mode("Append")
    ...
    hotel_logging_clean.createCassandraTable("testkeyspace2","hotel_logging")
    hotel_logging_clean
      .write
      .format("org.apache.spark.sql.cassandra")
      .mode("Append")
...
//      )
//
//    hotel_mapping_location_distance.createCassandraTable("testkeyspace2", "hotel_location")
...
//      .save()
    ...
    cosine_hotel_top_clean.createCassandraTable("testkeyspace2", "cosine_similar")
