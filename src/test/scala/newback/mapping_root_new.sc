import org.apache.spark.sql.functions.{col, collect_list, first}
import recommender.engine.core.Udf.mapProviderUdf
...
val mapping_root_suggest = mapping_root.withColumn("suggest",mapProviderUdf(col("url"), col("domain_id"), col("avg_price")))

val mapping_root_group_id = mapping_root_suggest
  .groupBy("id").agg(
  first(col("name")),
  collect_list("suggest")
)

mapping_root_group_id.show()

mapping_root_group_id.groupBy().count().show()
mapping_root_group_id.groupBy().count().show()
...
mapping_root_group_id.groupBy("id").count().orderBy(col("count").desc).show()


mapping_root_group_id.show(false)